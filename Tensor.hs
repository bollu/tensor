...
data M a where
 ...
 MD :: M a -> M a -> (T a -> M b) -> M b -- derivative
...
runcompiler :: CM a -> a
runcompiler cm = let (_, a) = runcm cm 0 in a
...
instance Monad CM where
  return x = CM $ \i -> (i, x)
  (CM cm) >>= f = CM $ \i -> let (i', a) = cm i; (i'', b) = runcm (f a) i' in (i'', b)

compileder :: T a -> T a -> CM (T a)
compileder = undefined
